  ...
  render() {
    ...
    if (this.state.search === "") {
      ...
      titleArray.forEach((title, i) => {
        if (
          title.toLowerCase().includes(this.state.search.toLocaleLowerCase())
        ) {
    ...
    const newList = filteredQuizDates.slice(
      page * rowsPerPage,
      page * rowsPerPage + rowsPerPage
    );
    const newTitles = filteredQuizTitles.slice(
      page * rowsPerPage,
      page * rowsPerPage + rowsPerPage
    );
    const List = newList.map((date, i) => {
      ...
      return (
        <TableRow id={date} key={id} className="tableItem">
          ...
          <TableCell onClick={this.handleClick}>{title}</TableCell>
        </TableRow>
      );
    });
            ...
            {dateArray.length === 0 ? (
              ...
              <div>
                ...
                <TablePagination
                  ...
                  count={
                    this.state.search !== ""
                      ? filteredQuizDates.length
                      : this.state.dateArray.length
                  }
