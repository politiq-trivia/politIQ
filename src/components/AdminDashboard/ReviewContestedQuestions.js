...
class ReviewContestedQuestions extends Component {
  ...
  skip = () => {
    ...
    // qVal is the value
    const userKeys = this.state.data[this.state.selectedQuizId][Object.keys(contestedQsForThisQuiz)[qNum]]; // eslint-disable-line max-len
                  ...
                  <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                    <Button color="primary">
                      <span
                        style={{ color: 'red' }}
                        onClick={this.reject}
                        onKeyDown={(e) => { if (e.keyCode === 13) { this.reject(); } } }
                        role='button'
                        tabIndex={0}
                      >
                        Reject Issue
