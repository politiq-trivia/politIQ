...
class LeaderboardWithContext extends Component {
  ...
  getPolitIQ = async (uid, timeframe) => {
    const politIQ = await getPolitIQ(uid, timeframe, this.context)
    return politIQ
  }
  ...
  render() {
    ...
    const renderMonthlyLeaders = rankingArray.map((stat, i) => {
      ...
      return (
        <NavLink style={{ textDecoration: 'none' }} to={`/profile/${stat[2]}`}><div className="leaderboard-object" key={i} >
        ...
        <div className="leaderboard-holder">
          ...
          <div className="leaderboard-right">
            <div className="leaderboard-tabs">
              <p onClick={this.toggleWeekly} className={this.state.weekly ? "weekly" : "weekly selected"}>Top Ten Monthly</p>
              <p onClick={this.toggleWeekly} className={this.state.weekly ? "weekly selected" : "weekly"}>Top Ten Weekly</p>
            </div>
            {this.state.noScores
...
LeaderboardWithContext.contextType = quizContext;
